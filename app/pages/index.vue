...
<template>
    <UContainer class="py-8 flex flex-col gap-y-4">
        <UInput
          ...
          :ui="{
            root: 'w-full',
            base: 'md:h-[5rem] md:!text-2xl md:min-w-[50rem] rounded-xl md:rounded-2xl ring-1 md:ring-2 ring-secondary',
          }"
        />
        ...
        <template #header>
          <div class="flex flex-col sm:flex-row justify-between gap-y-2 sm:gap-x-4">
            ...
            <div class="flex items-center gap-x-4">
              <UBadge icon="fluent-mdl2:video-search" :size="$device.isMobile ? 'md' : 'xl'">
                s{{ result.season.toString().padStart(2, "0") }}e{{ result.episode.toString().padStart(2, "0") }}
              </UBadge>
              <UBadge icon="fluent:video-person-clock-48-filled" :size="$device.isMobile ? 'md' : 'xl'">{{ result.displayStart }}</UBadge>
            </div>
          </div>
        </template>
        <div class="flex w-full justify-between overflow-x-auto gap-x-2 items-center">
            <NuxtImg
              class="rounded-xl w-full md:w-[32%] border-info border-2"
